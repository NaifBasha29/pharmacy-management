...
router.get('/stats', protect, authorize('admin'), asyncHandler(async (req, res) => {
    const [total, active, pending, inactive] = await Promise.all([
        ...
        Clinic.countDocuments({ 'verification.clinicStatus': 'pending_verification' }),
...
router.post('/',
    ...
    asyncHandler(async (req, res) => {
        ...
        if (clinicData.adminAccount && !clinicData.adminAccount.tempPassword) {
            clinicData.adminAccount.tempPassword = crypto.randomBytes(8).toString('hex');
        }
...
router.put('/:id',
    ...
    asyncHandler(async (req, res) => {
        ...
        if (!updateData.audit.modificationLogs) updateData.audit.modificationLogs = [];
...
router.put('/:id/status', protect, authorize('admin'), asyncHandler(async (req, res) => {
    ...
    if (adminAccountStatus) updateObj['verification.adminAccountStatus'] = adminAccountStatus;
    if (adminNotes !== undefined) updateObj['verification.adminNotes'] = adminNotes;
...
router.put('/:id/activate', protect, authorize('admin'), asyncHandler(async (req, res) => {
    const clinic = await Clinic.findById(req.params.id).select('+adminAccount.tempPassword');
    ...
    if (!adminUser) {
        ...
        const tempPassword = clinic.adminAccount.tempPassword || crypto.randomBytes(8).toString('hex');
...
// @access  Private/Admin
router.post('/:id/send-credentials', protect, authorize('admin'), asyncHandler(async (req, res) => {
    const clinic = await Clinic.findById(req.params.id).select('+adminAccount.tempPassword');
    ...
    res.json({
        ...
        data: {
            ...
            credentials: process.env.NODE_ENV === 'development' ? credentials : undefined
        }
    });
}));
