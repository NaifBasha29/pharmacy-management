...
router.put(
  ...
  validateObjectId,
  asyncHandler(async (req, res) => {
    const { action, rejectionReason, medicines, validUntil, pharmacistNote } =
      req.body;
    ...
    if (action === "verify") {
      ...
      if (pharmacistNote) prescription.notes = pharmacistNote;
      ...
      if (pharmacistNote && !rejectionReason)
        prescription.notes = pharmacistNote;
    } else {
      return res.status(400).json({
        success: false,
