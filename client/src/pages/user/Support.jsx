...
const Support = () => {
    ...
    const btn = { padding: '0.875rem 1.5rem', fontWeight: 600, color: 'white', background: 'linear-gradient(135deg, #3b82f6, #2563eb)', border: 'none', borderRadius: '0.75rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' };
            ...
            <main className="dashboard-main" style={page}>
                ...
                <div style={{ marginBottom: '2rem' }}>
                    <h1 style={{ fontSize: '1.875rem', fontWeight: 700, color: '#1e293b' }}>
                        Help & <span style={{ background: 'linear-gradient(135deg, #3b82f6, #2563eb)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Support</span>
                    </h1>
                    <p style={{ color: '#64748b' }}>Get help with your orders, prescriptions, and account</p>
                </div>

                {/* Contact Cards */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
                    <div style={{ ...card, padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{ width: 56, height: 56, background: 'linear-gradient(135deg, #3b82f6, #2563eb)', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '1.5rem' }}><FiPhone /></div>
                        <div>
                            <div style={{ fontWeight: 700, color: '#1e293b' }}>Call Us</div>
                            <div style={{ color: '#64748b', fontSize: '0.875rem' }}>1800-123-4567</div>
                            <div style={{ fontSize: '0.75rem', color: '#10b981' }}>24/7 Available</div>
                        </div>
                    </div>
                    <div style={{ ...card, padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{ width: 56, height: 56, background: 'linear-gradient(135deg, #10b981, #059669)', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '1.5rem' }}><FiMail /></div>
                        <div>
                            <div style={{ fontWeight: 700, color: '#1e293b' }}>Email</div>
                            <div style={{ color: '#64748b', fontSize: '0.875rem' }}>support@pharmacare.com</div>
                    ...
                    <div style={{ ...card, padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <div>
                            <div style={{ fontWeight: 700, color: '#1e293b' }}>Live Chat</div>
                            <div style={{ color: '#64748b', fontSize: '0.875rem' }}>Chat with an agent</div>
                ...
                {/* Tabs */}
                <div style={{ display: 'inline-flex', background: 'white', borderRadius: '0.75rem', padding: '0.375rem', gap: '0.25rem', marginBottom: '2rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                    {[{ id: 'faq', label: 'FAQs', icon: <FiHelpCircle /> }, { id: 'ticket', label: 'Submit Ticket', icon: <FiSend /> }].map(tab => (
                        <button key={tab.id} onClick={() => setActiveSection(tab.id)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 1.25rem', fontSize: '0.875rem', fontWeight: 600, color: activeSection === tab.id ? '#2563eb' : '#64748b', background: activeSection === tab.id ? '#eff6ff' : 'transparent', border: 'none', borderRadius: '0.5rem', cursor: 'pointer' }}>
                            {tab.icon} {tab.label}
                        </button>
                    ))}
                </div>
                ...
                {activeSection === 'faq' && (
                    <div style={card}>
                        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #f1f5f9' }}>
                            <h3 style={{ fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><FiHelpCircle style={{ color: '#3b82f6' }} /> Frequently Asked Questions</h3>
                        </div>
                            ...
                            {faqs.map((faq, i) => (
                                <div key={i} style={{ borderBottom: i < faqs.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                                    <button onClick={() => setExpandedFaq(expandedFaq === i ? null : i)} style={{ width: '100%', padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                                        <span style={{ fontWeight: 600, color: '#1e293b' }}>{faq.q}</span>
                                        {expandedFaq === i ? <FiChevronUp style={{ color: '#3b82f6' }} /> : <FiChevronDown style={{ color: '#9ca3af' }} />}
                                    </button>
                                    <div style={{ maxHeight: expandedFaq === i ? '200px' : '0', overflow: 'hidden', transition: 'max-height 0.3s ease' }}>
                                        <div style={{ padding: '0 1rem 1rem', color: '#64748b', fontSize: '0.9375rem', lineHeight: 1.6 }}>{faq.a}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeSection === 'ticket' && (
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
                        <div style={card}>
                            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #f1f5f9' }}>
                                <h3 style={{ fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><FiSend style={{ color: '#3b82f6' }} /> Create Support Ticket</h3>
                            </div>
                        ...
                        <div>
                            <div style={{ ...card, padding: '1.5rem' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: '#f9fafb', borderRadius: '0.5rem' }}>
                                        <span style={{ color: '#64748b' }}>Order Issues</span>
                                        <span style={{ fontWeight: 600, color: '#10b981' }}>2-4 hours</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: '#f9fafb', borderRadius: '0.5rem' }}>
                                        <span style={{ color: '#64748b' }}>Prescription</span>
                                        <span style={{ fontWeight: 600, color: '#10b981' }}>1-2 hours</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: '#f9fafb', borderRadius: '0.5rem' }}>
                            ...
                            <div style={{ ...card, padding: '1.5rem', marginTop: '1rem', background: 'linear-gradient(135deg, #ecfdf5, #d1fae5)' }}>
                                ...
                                <p style={{ fontSize: '0.875rem', color: '#047857', lineHeight: 1.5 }}>Medical emergencies get instant priority. Call our hotline for urgent assistance.</p>
                            </div>
                        </div>
                    </div>
                )}
