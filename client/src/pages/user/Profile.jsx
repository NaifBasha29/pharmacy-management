...
import { usersAPI, authAPI, patientsAPI } from '../../services/api';
import Sidebar from '../../components/common/Sidebar';
import { FiUser, FiActivity, FiShield, FiLock, FiX, FiHeart, FiSave } from 'react-icons/fi';
import toast from 'react-hot-toast';
...
const Profile = () => {
    ...
    const [personalData, setPersonalData] = useState({
        ...
        street: addrObj.street || (typeof user?.address === 'string' ? user.address : ''),
        city: addrObj.city || '', state: addrObj.state || '', zipCode: addrObj.zipCode || ''
    });
            ...
            try {
                ...
                if (u) {
                    setMedicalData({
                        ...
                        chronicConditions: Array.isArray(u.chronicConditions) ? u.chronicConditions : []
                    });
                    if (u.address && typeof u.address === 'object') {
                        setPersonalData(prev => ({
                            ...prev, street: u.address.street || prev.street,
                            city: u.address.city || prev.city, state: u.address.state || prev.state,
                            zipCode: u.address.zipCode || prev.zipCode
                        }));
                    }
                }
            } catch { /* ignore */ }
    ...
    const handlePersonalSubmit = async (e) => {
        ...
        try {
            await usersAPI.updateProfile({
                name: personalData.name, email: personalData.email, phone: personalData.phone,
                address: { street: personalData.street, city: personalData.city, state: personalData.state, zipCode: personalData.zipCode }
            });
            toast.success('Profile updated!');
    ...
    const handleMedicalSubmit = async () => {
        ...
        try {
            await patientsAPI.update(user.id, { bloodGroup: medicalData.bloodGroup, allergies: medicalData.allergies, chronicConditions: medicalData.chronicConditions });
            toast.success('Health profile saved!');
        } catch (error) { toast.error(error.response?.data?.message || 'Failed to save health profile'); }
...
=======
        ...
        } catch { toast.error('Change failed'); } finally { setLoading(false); }
    };

    const handleArrayInput = (e, field) => {
        if (e.key === 'Enter') { e.preventDefault(); const v = e.target.value.trim();
            if (v) { setMedicalData(prev => ({ ...prev, [field]: [...prev[field], v] })); e.target.value = ''; }
        }
    };
    ...
    const lbl = { display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#e5e5e5', marginBottom: '0.5rem' };
    ...
    const tabs = [
        ...
        { id: 'medical', label: 'Health Profile', icon: <FiActivity /> },
    ...
    return (
        <div className="dashboard-layout">
            ...
            <main className="dashboard-main" style={page}>
                <div style={{ marginBottom: '2rem' }}>
                    <h1 style={{ fontSize: '1.875rem', fontWeight: 700, color: '#fff' }}>My <span style={{ background: 'linear-gradient(135deg,#f97316,#ea580c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Profile</span></h1>
                    <p style={{ color: '#9ca3af' }}>Manage your account and health information</p>
                </div>
                <div style={{ display: 'flex', gap: '2rem' }}>
                    <div style={{ width: 280, flexShrink: 0 }}>
                        <div style={card}><div style={{ padding: '1rem' }}>
                            {tabs.map(tab => (
                                <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem', marginBottom: '0.5rem', border: 'none', borderRadius: '0.75rem', cursor: 'pointer', background: activeTab === tab.id ? 'linear-gradient(135deg,#f97316,#ea580c)' : 'transparent', color: activeTab === tab.id ? 'white' : '#9ca3af', fontWeight: 600 }}>
                                    <span style={{ fontSize: '1.25rem' }}>{tab.icon}</span>{tab.label}
                                </button>
                            ))}
                        </div></div>
                            ...
                            {activeTab === 'personal' && (
                                <form onSubmit={handlePersonalSubmit}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '1rem' }}>
                                        <div><label style={lbl}>Full Name</label><input type="text" value={personalData.name} onChange={e => setPersonalData({ ...personalData, name: e.target.value })} style={input} /></div>
                                        <div><label style={lbl}>Email</label><input type="email" value={personalData.email} disabled style={{ ...input, background: '#e5e7eb', color: '#9ca3af' }} /></div>
                                        <div><label style={lbl}>Phone</label><input type="tel" value={personalData.phone} onChange={e => setPersonalData({ ...personalData, phone: e.target.value })} style={input} /></div>
                                        <div><label style={lbl}>Street</label><input type="text" value={personalData.street} onChange={e => setPersonalData({ ...personalData, street: e.target.value })} style={input} placeholder="Street address" /></div>
                                        <div><label style={lbl}>City</label><input type="text" value={personalData.city} onChange={e => setPersonalData({ ...personalData, city: e.target.value })} style={input} placeholder="City" /></div>
                                        <div><label style={lbl}>State</label><input type="text" value={personalData.state} onChange={e => setPersonalData({ ...personalData, state: e.target.value })} style={input} placeholder="State" /></div>
                                        <div><label style={lbl}>ZIP Code</label><input type="text" value={personalData.zipCode} onChange={e => setPersonalData({ ...personalData, zipCode: e.target.value })} style={input} placeholder="ZIP code" /></div>
                                    </div>
                            ...
                            {activeTab === 'medical' && (
                                <div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: '#fef2f2', borderRadius: '0.75rem', marginBottom: '1.5rem' }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <label style={{ ...lbl, marginBottom: '0.25rem' }}>Blood Group</label>
                                    ...
                                    <div style={{ marginBottom: '1rem' }}>
                                        <label style={lbl}>Allergies (Press Enter)</label>
                                    ...
                                    <div style={{ marginBottom: '1.5rem' }}>
                                        <label style={lbl}>Chronic Conditions (Press Enter)</label>
                                    ...
                                    <div style={{ textAlign: 'right', paddingTop: '1rem', borderTop: '1px solid #f1f5f9' }}>
                                        <button type="button" onClick={handleMedicalSubmit} disabled={medicalLoading} style={{ ...btn, display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
                                            <FiSave /> {medicalLoading ? 'Saving...' : 'Save Health Profile'}
                                        </button>
                                    </div>
                                </div>
                            )}
                            {activeTab === 'security' && (
                                <form onSubmit={handlePasswordSubmit}>
                                    ...
                                    <div style={{ maxWidth: 400 }}>
                                        <div style={{ marginBottom: '1rem' }}><label style={lbl}>Current Password</label><div style={{ position: 'relative' }}><input type="password" value={passwordData.currentPassword} onChange={e => setPasswordData({ ...passwordData, currentPassword: e.target.value })} style={input} required /><FiLock style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af' }} /></div></div>
                                        <div style={{ marginBottom: '1rem' }}><label style={lbl}>New Password</label><input type="password" value={passwordData.newPassword} onChange={e => setPasswordData({ ...passwordData, newPassword: e.target.value })} style={input} required /></div>
                                        <div style={{ marginBottom: '1rem' }}><label style={lbl}>Confirm Password</label><input type="password" value={passwordData.confirmPassword} onChange={e => setPasswordData({ ...passwordData, confirmPassword: e.target.value })} style={input} required /></div>
                                    </div>
                                    <div style={{ textAlign: 'right', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid #f1f5f9' }}>
