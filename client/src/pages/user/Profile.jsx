...
const Profile = () => {
    ...
    const input = { width: '100%', padding: '0.75rem 1rem', border: '1px solid var(--border-light)', borderRadius: '0.75rem', background: 'var(--bg-tertiary)', outline: 'none' };
        ...
        <div className="dashboard-layout">
            ...
            <main className="dashboard-main" style={page}>
                <div style={{ marginBottom: '2rem' }}>
                    <h1 style={{ fontSize: '1.875rem', fontWeight: 700, color: 'var(--text-primary)' }}>My <span style={{ background: 'linear-gradient(135deg, #f97316, #ea580c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Profile</span></h1>
                    <p style={{ color: 'var(--text-secondary)' }}>Manage your account and health information</p>
                </div>
                    ...
                    <div style={{ width: 280, flexShrink: 0 }}>
                        <div style={card}>
                            <div style={{ padding: '1rem' }}>
                                {tabs.map(tab => (
                                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem', marginBottom: '0.5rem', border: 'none', borderRadius: '0.75rem', cursor: 'pointer', background: activeTab === tab.id ? 'linear-gradient(135deg, #f97316, #ea580c)' : 'transparent', color: activeTab === tab.id ? 'white' : 'var(--text-secondary)', fontWeight: 600, transition: 'all 0.2s' }}>
                        ...
                        <div style={{ ...card, marginTop: '1rem', padding: '1.5rem', textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>{user?.role}</div>
                        ...
                        <div style={{ ...card, padding: '2rem' }}>
                            {activeTab === 'personal' && (
                                <form onSubmit={handlePersonalSubmit}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: '1px solid var(--bg-secondary)' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
                                        ...
                                        <div><label style={label}>Email</label><input type="email" value={personalData.email} disabled style={{ ...input, background: 'var(--border-light)', color: 'var(--text-secondary)' }} /></div>
                                        ...
                                        <div><label style={label}>Street</label><input type="text" value={personalData.address.street} onChange={e => setPersonalData({ ...personalData, address: { ...personalData.address, street: e.target.value } })} style={input} /></div>
                                        <div><label style={label}>City</label><input type="text" value={personalData.address.city} onChange={e => setPersonalData({ ...personalData, address: { ...personalData.address, city: e.target.value } })} style={input} /></div>
                                        <div><label style={label}>State</label><input type="text" value={personalData.address.state} onChange={e => setPersonalData({ ...personalData, address: { ...personalData.address, state: e.target.value } })} style={input} /></div>
                                        <div><label style={label}>ZIP</label><input type="text" value={personalData.address.zipCode} onChange={e => setPersonalData({ ...personalData, address: { ...personalData.address, zipCode: e.target.value } })} style={input} /></div>
                                    </div>
                                    <div style={{ textAlign: 'right', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--bg-secondary)' }}>
                            ...
                            {activeTab === 'medical' && (
                                <div>
                                    ...
                                    <div style={{ textAlign: 'right', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--bg-secondary)' }}>
                                        <button type="button" onClick={handleMedicalSave} disabled={loading} style={btn}>{loading ? 'Saving...' : 'Save Health Profile'}</button>
                                    </div>
                                </div>
                            )}
