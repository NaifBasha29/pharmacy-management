...
import { prescriptionsAPI, API_HOST } from '../../services/api';
...
const Prescriptions = () => {
    ...
    const badge = (s) => ({ padding: '0.25rem 0.625rem', fontSize: '0.7rem', fontWeight: 600, borderRadius: '9999px', background: s === 'approved' ? 'var(--status-success-bg)' : s === 'rejected' ? 'var(--status-error-bg)' : 'var(--status-warning-bg)', color: s === 'approved' ? 'var(--status-success)' : s === 'rejected' ? 'var(--status-error)' : 'var(--status-warning)' });

    // Resolve prescription file paths to absolute URLs (backend may store relative paths).
    const resolveImageUrl = (prescription) => {
        const placeholder = 'https://placehold.co/400x160?text=Rx';
        const img = prescription.imageUrl || prescription.image || prescription.file || '';
        ...
        <div className="dashboard-layout no-top-nav">
            ...
            <main className="dashboard-main" style={page}>
                <div style={{ marginBottom: '2rem' }}>
                    <h1 style={{ fontSize: '1.875rem', fontWeight: 700, color: 'var(--text-primary)' }}>My <span className="ud-gradient-text" style={{ WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Prescriptions</span></h1>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '2rem' }}>
                    ...
                    <div style={card}>
                        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--bg-secondary)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            <div style={{ width: 40, height: 40, background: 'var(--gradient-purple)', borderRadius: '0.625rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}><FiUpload /></div>
                        ...
                        <form onSubmit={handleUpload} style={{ padding: '1.5rem' }}>
                            <div style={{ border: '2px dashed var(--border-light)', borderRadius: '0.75rem', padding: previewUrl ? 0 : '2rem', textAlign: 'center', background: 'var(--bg-tertiary)', position: 'relative', cursor: 'pointer' }}>
                                ...
                                {previewUrl ? (
                                    <>
                                        ...
                                        <button type="button" onClick={() => { setSelectedFile(null); setPreviewUrl(null); }} style={{ position: 'absolute', top: 8, right: 8, width: 28, height: 28, background: 'var(--status-error)', color: 'white', border: 'none', borderRadius: '50%', cursor: 'pointer' }}><FiX size={14} /></button>
                                    </>
                                ) : (
                                    <>
                                        <div style={{ width: 56, height: 56, background: 'var(--accent-purple-50)', borderRadius: '50%', margin: '0 auto 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-purple-500)', fontSize: '1.5rem' }}><FiImage /></div>
                            ...
                            <button type="submit" disabled={!selectedFile || uploading} className="btn btn-primary" style={{ width: '100%', marginTop: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', padding: '1.5rem' }}>
                                {prescriptions.map(p => (
                                    <div key={p._id} style={{ background: 'var(--bg-tertiary)', borderRadius: '0.75rem', overflow: 'hidden', border: '1px solid var(--border-light)' }}>
                                        <img src={resolveImageUrl(p)} alt="Rx" style={{ width: '100%', height: 140, objectFit: 'cover' }} />
                                        <div style={{ padding: '1rem' }}>
                                            ...
                                            {p.pharmacistNote && <div style={{ fontSize: '0.8rem', background: 'var(--primary-50)', padding: '0.5rem', borderRadius: '0.5rem', borderLeft: '3px solid var(--primary-500)' }}>{p.pharmacistNote}</div>}
                                        </div>
                                    </div>
                                ))}
                            </div>
