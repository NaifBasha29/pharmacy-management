      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            setFavorites(prev => prev.filter(f => f.medicine?._id !== medicineId));
          } catch (error) {
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={styles.card}>
        <Image
          source={{ uri: med.image || 'https://via.placeholder.com/80?text=Rx' }}
          style={styles.image}
        />
        <View style={styles.info}>
          <Text style={styles.name} numberOfLines={2}>{med.name}</Text>
          {med.dosageForm && (
            <Text style={styles.dosage}>{med.dosageForm} {med.strength || ''}</Text>
          )}
          <View style={styles.priceRow}>
            <Text style={styles.price}>${discountedPrice.toFixed(2)}</Text>
            {med.discount > 0 && (
              <Text style={styles.originalPrice}>${(med.price || 0).toFixed(2)}</Text>
            )}
          </View>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {favorites.length === 0 ? (
        <View style={styles.center}>
          ...
          <Text style={styles.emptySubtitle}>Save medicines you like for later</Text>
          <TouchableOpacity
...
const createStyles = (theme) => StyleSheet.create({
  ...
  image: { width: 70, height: 70, borderRadius: 8, backgroundColor: theme.inputBackground },
  ...
  originalPrice: { fontSize: 12, color: theme.textTertiary, textDecorationLine: 'line-through', marginLeft: 6 },
