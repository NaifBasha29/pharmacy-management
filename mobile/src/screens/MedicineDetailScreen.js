...
export default function MedicineDetailScreen({ route, navigation }) {
  ...
  const fetchMedicine = async () => {
    try {
      ...
      if (res.data.success) {
        // API returns { success: true, data: { medicine } }
  ...
  const handleAddToCart = () => {
    ...
    Alert.alert(
      "Added to Cart",
      `${medicine.name} x${quantity} added to your cart`,
      [
  ...
  const lowStock =
    medicine.stock > 0 && medicine.stock <= (medicine.minStockLevel || 10);
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Image
          source={{
            uri:
              medicine.image || "https://via.placeholder.com/300?text=Medicine",
          }}
          style={styles.image}
          resizeMode="contain"
        />
        ...
        {lowStock && !outOfStock && (
          <View
            style={[styles.stockAlert, { backgroundColor: theme.warningMuted }]}
          >
...
>>>>>>> 8a0117a (Rebase and fixes functionality)
        ...
        <View style={styles.section}>
          ...
          {medicine.uses && medicine.uses.length > 0 ? (
            medicine.uses.map((use, i) => (
              <View key={`use-${i}`} style={styles.listItem}>
            ...
            <View style={styles.listItem}>
              ...
              <Text style={styles.listText}>
                Treats various medical conditions as prescribed by healthcare
                professionals
              </Text>
            </View>
          )}
        ...
        <View style={styles.section}>
          ...
          {medicine.sideEffects && medicine.sideEffects.length > 0 ? (
            medicine.sideEffects.map((effect, i) => (
              <View key={`effect-${i}`} style={styles.listItem}>
                <Icon
        ...
        <View style={styles.section}>
          ...
          <View style={styles.listItem}>
            <Icon name="information" size={16} color={theme.primary} />
            <Text style={styles.listText}>
              Seek immediate medical attention if overdose occurs
            </Text>
          </View>
        </View>
