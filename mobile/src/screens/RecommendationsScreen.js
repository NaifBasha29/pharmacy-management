  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={styles.card}>
        <Image
          source={{ uri: item.image || 'https://via.placeholder.com/80?text=Rx' }}
          style={styles.image}
        />
        <View style={styles.info}>
          <Text style={styles.name} numberOfLines={2}>{item.name}</Text>
          {item.dosageForm && (
            <Text style={styles.dosage}>{item.dosageForm} {item.strength || ''}</Text>
          )}
          {item.category?.name && (
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={() => { setRefreshing(true); fetchRecs(); }}
            tintColor={theme.primary}
          />
        }
        ListEmptyComponent={
          <View style={styles.center}>
            <Icon name="thumb-up-outline" size={60} color={theme.textTertiary} />
...
const createStyles = (theme) => StyleSheet.create({
  ...
  image: { width: 60, height: 60, borderRadius: 8, backgroundColor: theme.inputBackground },
  ...
  oldPrice: { fontSize: 12, color: theme.textTertiary, textDecorationLine: 'line-through' },
  discountBadge: { backgroundColor: theme.successMuted, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  discountText: { fontSize: 10, color: theme.success, fontWeight: '700' },
  addBtn: {
    backgroundColor: theme.primary, borderRadius: 22,
    width: 44, height: 44, justifyContent: 'center', alignItems: 'center', marginLeft: 8,
  },
