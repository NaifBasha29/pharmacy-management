...
import {
  ...
  Alert, ActivityIndicator, Modal, TextInput, Share
} from 'react-native';
...
export default function OrderDetailScreen({ route, navigation }) {
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            const msg = error.response?.data?.message || 'Failed to create refill';
  ...
  const handleSubmitReview = async () => {
    try {
      setSubmitting(true);
      const res = await reviewsAPI.create(orderId, reviewForm.rating, reviewForm.comment);
      if (res.data.success) {
        setReview(res.data.data);
        setShowReview(false);
                  ...
                  <View key={step} style={styles.timelineStep}>
                    <View style={styles.timelineLeft}>
                      ...
                      {index < STATUS_STEPS.length - 1 && (
                        <View style={[styles.line, isActive && currentStep > index && styles.activeLine]} />
                      )}
                    </View>
        ...
        <View style={styles.section}>
          ...
          {order.items?.map((item, index) => (
            <View key={index} style={styles.itemRow}>
              ...
              <Text style={styles.itemTotal}>${((item.price || 0) * (item.quantity || 0)).toFixed(2)}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.priceRow}>
            ...
            <Text style={styles.priceValue}>
              {(order.shippingCost || 0) === 0 ? 'FREE' : `$${order.shippingCost.toFixed(2)}`}
            </Text>
          </View>
          ...
          <View style={styles.priceRow}>
            ...
            <Text style={styles.priceValue}>{order.paymentMethod?.toUpperCase() || 'COD'}</Text>
          </View>
          <View style={styles.priceRow}>
...
const createStyles = (theme) => StyleSheet.create({
  ...
  dot: {
    width: 24, height: 24, borderRadius: 12, backgroundColor: theme.surfaceHighlight,
    borderWidth: 2, borderColor: theme.border, justifyContent: 'center', alignItems: 'center',
  },
  ...
  line: { flex: 1, width: 2, backgroundColor: theme.border, marginVertical: 2 },
  ...
  itemRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: theme.border + '50',
  },
  itemInfo: { flex: 1 },
  ...
  cancelBtn: {
    ...
    padding: 14, borderRadius: 12, borderWidth: 1, borderColor: theme.error,
  },
  cancelBtnText: { color: theme.error, fontSize: 15, fontWeight: '600' },
  refillBtn: {
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8,
    padding: 14, borderRadius: 12, backgroundColor: theme.primary,
  },
  refillBtnText: { color: '#fff', fontSize: 15, fontWeight: '600' },
  reviewDisplay: { gap: 8 },
  stars: { flexDirection: 'row', gap: 2 },
  reviewComment: { fontSize: 14, color: theme.textSecondary, fontStyle: 'italic' },
  reviewBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    padding: 12, borderRadius: 10, backgroundColor: theme.primaryMuted,
  },
  ...
  starsInput: { flexDirection: 'row', justifyContent: 'center', gap: 8, marginBottom: 20 },
  reviewInput: {
    backgroundColor: theme.inputBackground, borderRadius: 12, padding: 14,
    color: theme.textPrimary, fontSize: 15, minHeight: 100, marginBottom: 16,
    borderWidth: 1, borderColor: theme.border,
  },
  submitBtn: {
    backgroundColor: theme.primary, borderRadius: 12, paddingVertical: 14, alignItems: 'center',
  },
